...
const double precision = 1e-5 + eps;

bool significantlyDifferent(double a, double b)
{
    if (fabs(a-b)<precision)
        return false;
    if (fabs(a-b)/(fabs(a)+fabs(b)+eps) < precision)
        return false;
    return true;
}
...
int main(int argc, char **args){
    ...
    while(true){
        ...
        if(flag1 && flag2)
        {
            ...
            if(significantlyDifferent(val1,val2)){
                printf("0\n");
